...
export const getMe = async (): Promise<GetMeResponse> => {
  ...
  try {
    await analytics().logEvent("get_me");
    ...
    if (!response.ok) {
      console.error(
        `[authService] getMe API failed with status ${response.status}.`
      );
    ...
    try {
      ...
      if (!responseData.data) {
        console.error(
          '[authService] getMe successful response but missing "data" field in JSON:',
          responseData
        );
        return {
          success: false,
          message: 'Successful response but missing "data" field.',
      ...
      console.error(
        "[authService] getMe JSON parsing failed even though status was ok:",
        jsonParseError
      );
      return {
        success: false,
    ...
    if (error instanceof SessionExpiredError) {
      ...
      console.log(
        "[authService] getMe caught SessionExpiredError, redirect initiated."
      );
...
export const logIn = async (
  ...
  try {
    await analytics().logEvent("log_in", {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "[authService] LogIn successful (status ok) but response body was not valid JSON or was empty. Assuming success based on status."
        );
        return {
          success: true,
          message: "Login successful (server response was not JSON).",
        };
      }
      console.error(
        "[authService] LogIn JSON parsing failed even though status was ok:",
        jsonParseError
      );
      return {
        success: false,
    ...
    return {
      success: false,
      message: "An unexpected network error occurred during login.",
    };
  }
};
...
export const updateUser = async (
  ...
  console.log(
    `[authService] Attempting to update user ${userId} at:`,
    requestUrl
  );
  ...
  try {
    ...
    const responseText = await response.text();
    console.log(
      `[authService] UpdateUser Response Status for user ${userId}:`,
      response.status
    );

    if (!response.ok) {
      console.error(
        `[authService] UpdateUser API failed for user ${userId} with status ${response.status}.`
      );
      try {
        const errorData = JSON.parse(responseText);
        return {
    ...
    try {
      ...
      if (response.status === 204) {
        ...
        console.log(
          `[authService] User ${userId} updated successfully (204 No Content).`
        );
        return {
          success: true,
          message: "User updated successfully (No Content)",
        };
      }
      if (
        responseText.trim() === "" ||
        ...
        console.warn(
          "[authService] UpdateUser successful (status ok) but response body was not valid JSON or was empty. Assuming success based on status."
        );
        return {
          success: true,
          message: "User updated successfully (server response was not JSON).",
        };
      }
      console.error(
        "[authService] UpdateUser JSON parsing failed even though status was ok:",
        jsonParseError
      );
      return {
        success: false,
    ...
    console.error(
      `[authService] UpdateUser network error or other issue for user ${userId}:`,
      error
    );
    if (error instanceof Error) {
      return {
        success: false,
        message: "A network error occurred during user update.",
        error: error.message,
      };
    }
    return {
      success: false,
      message: "An unexpected network error occurred during user update.",
    };
  }
};
...
export const logOut = async (): Promise<LogOutResponse> => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[authService] LogOut API failed with status ${response.status}.`
      );
      try {
        const errorData = JSON.parse(responseText);
        return {
    ...
    try {
      ...
      if (response.status === 204 || responseText.trim() === "") {
        console.log(
          "[authService] Logout successful (204 No Content or empty response)."
        );
      ...
      if (
        ...
        console.warn(
          "[authService] LogOut successful (status ok) but response body was not valid JSON. Assuming success based on status and text."
        );
        return {
          success: true,
          message: "Logout successful (server response was not JSON).",
        };
      }
      console.error(
        "[authService] LogOut JSON parsing failed even though status was ok:",
        jsonParseError
      );
      return {
        success: false,
    ...
    if (error instanceof Error) {
      return {
        success: false,
        message: "A network error occurred during logout.",
        error: error.message,
      };
    }
    return {
      success: false,
      message: "An unexpected network error occurred during logout.",
    };
  }
};
...
export const changePassword = async (
  ...
  try {
    ...
    const responseText = await response.text();
    console.log(
      "[authService] ChangePassword Response Status:",
      response.status
    );
    ...
    if (!response.ok) {
      console.error(
        `[authService] ChangePassword API failed with status ${response.status}.`
      );
      try {
        const errorData = JSON.parse(responseText);
        return {
    ...
    try {
      ...
      if (response.status === 204 || responseText.trim() === "") {
        console.log(
          "[authService] Password changed successfully (204 No Content or empty response)."
        );
      ...
      if (
        ...
        console.warn(
          "[authService] ChangePassword successful (status ok) but response body was not valid JSON. Assuming success based on status and text."
        );
        return {
          success: true,
          message:
            "Password changed successfully (server response was not JSON).",
        };
      }
      console.error(
        "[authService] ChangePassword JSON parsing failed even though status was ok:",
        jsonParseError
      );
      return {
        success: false,
        message:
          "Failed to parse successful server response for password change.",
        error: (jsonParseError as Error).message,
        rawErrorResponse: responseText.substring(0, 500),
      };
    }
    ...
    console.error(
      "[authService] ChangePassword network error or other issue:",
      error
    );
    if (error instanceof Error) {
      return {
        success: false,
        message: "A network error occurred during password change.",
        error: error.message,
      };
    }
    return {
      success: false,
      message: "An unexpected network error occurred during password change.",
    };
  }
};
...
export const refreshToken = async (
  ...
  try {
    await analytics().logEvent("refresh_token");
    const response = await fetch(requestUrl, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
    ...
    if (!response.ok) {
      console.error(
        `[authService] RefreshToken API failed with status ${response.status}.`
      );
      try {
        const errorData = JSON.parse(responseText);
        return {
    ...
    try {
      ...
      // This implies HttpOnly cookies might have been updated, but no tokens in body.
        ...
        console.log(
          "[authService] RefreshToken HTTP call successful (data: null), but no new tokens in response body."
        );
        return {
      ...
      // Case 3: Response is OK, but structure is not as expected for tokens.
        ...
        console.error(
          "[authService] RefreshToken successful HTTP response but missing accessToken or unexpected data structure:",
          responseData
        );
        return {
          success: false, // Treat as failure to obtain new token from body
          message:
            "Token refresh response missing accessToken in body or unexpected structure.",
      ...
      console.error(
        "[authService] RefreshToken JSON parsing failed even though status was ok:",
        jsonParseError
      );
      return {
        success: false,
        message:
          "Failed to parse successful server response for token refresh.",
        error: (jsonParseError as Error).message,
        rawErrorResponse: responseText.substring(0, 500),
      };
    }
    ...
    console.error(
      "[authService] RefreshToken network error or other issue:",
      error
    );
    if (error instanceof Error) {
      return {
        success: false,
        message: "A network error occurred during token refresh.",
        error: error.message,
      };
    }
    return {
      success: false,
      message: "An unexpected network error occurred during token refresh.",
    };
  }
};
