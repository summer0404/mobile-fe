...
} from "@/services/authService"; // Assuming
// these exist
      ...
      try {
        ...
        if (refreshResponse.success && refreshResponse.data?.accessToken) {
          ...
          if (refreshResponse.data.refreshToken) {
            // If your refresh API also returns a new refresh token
            await AsyncStorage.setItem(
              REFRESH_TOKEN_KEY,
          ...
          console.log(
            ...
            refreshResponse.message || refreshResponse.error
          );
          return false;
        }
      } catch (error) {
    ...
    const checkStatus = async () => {
      try {
        ...
        if (userDetailsResponse.success && userDetailsResponse.data) {
          ...
          console.log(
            "Access token valid. User details fetched. Redirecting to home."
          );
          router.replace("/home");
        } else if (
          userDetailsResponse.error?.toLowerCase().includes("unauthorized") ||
          userDetailsResponse.message?.toLowerCase().includes("unauthorized") ||
          (userDetailsResponse.rawErrorResponse &&
            JSON.parse(userDetailsResponse.rawErrorResponse).statusCode === 401)
        ) {
          ...
          if (refreshedSuccessfully) {
            ...
            console.log(
              "Token refresh failed. Clearing tokens and redirecting to signIn."
            );
          ...
          console.warn(
            "getMe failed with non-auth error or no data:",
            userDetailsResponse.message || userDetailsResponse.error
          );
        ...
        try {
          ...
          console.error(
            "Failed to remove tokens on critical error:",
            removeError
          );
        }
        router.replace("/auth/signIn");
